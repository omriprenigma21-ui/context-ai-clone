...
import { InlineArtifactViewer } from "@/components/artifacts/InlineArtifactViewer";
...
const DriveSection = ({ panelWidth = 50 }: DriveSectionProps) => {
  const { state } = useApp();
  const [currentView, setCurrentView] = useState("creations"); // "creations", "projects", or "artifact-viewer"
  ...
  return (
    <div className="bg-white relative h-full w-full overflow-hidden shadow-[-0.5px_0_0_0_rgba(0,0,0,0.1)]">
      ...
      {currentView === "artifact-viewer" && selectedArtifact ? (
        // Full Drive Panel Artifact Viewer - Takes complete control
        <InlineArtifactViewer
          artifact={selectedArtifact}
          onBack={handleBackToCreations}
          onFullscreen={handleFullscreenView}
        />
      ) : (
        // Default Drive View (Header + Creations + Projects)
