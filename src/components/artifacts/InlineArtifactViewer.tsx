...
import { AdvancedLinearArtifactViewer } from "@/components/AdvancedLinearArtifactViewer";
import {
  ChevronLeft,
  Share,
  Expand,
...
interface InlineArtifactViewerProps {
  artifact: {
    id: string;
    name: string;
    type: "spreadsheet" | "document" | "presentation" | "code" | "image" | "data" | "project";
    createdAt: string;
    size?: string;
    data?: unknown;
  };
        ...
        return (
          <AdvancedLinearArtifactViewer
            data={(artifact.data as any) || {
              ...
              assignees: [
                { name: "fouad.omri", initials: "FO", issueCount: 1, isOnline: true },
                { name: "backend_team", initials: "BT", issueCount: 2, isOnline: false }
              ]
            }}
            showHeader={true}
            showSidebar={true}
            onIssueUpdate={(issueId, updates) => {
              console.log(`Issue ${issueId} updated:`, updates);
            }}
            className="h-full"
          />
        );
