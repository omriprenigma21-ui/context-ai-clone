...
export const ImageArtifact = ({ artifact, size = "large", onClick }: ImageArtifactProps) => {
  return (
    <Card
      className={`${
        size === "small" ? "p-3" : "p-4"
    ...
    >
      <div className="flex items-start space-x-3">
        ...
        } bg-pink-50 rounded-lg flex items-center justify-center flex-shrink-0`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center space-x-3 text-xs text-gray-500">
            ...
            {artifact.metadata?.dimensions && (
              <>
                <span>•</span>
                <span>{artifact.metadata.dimensions}</span>
              </>
            )}
          </div>
          ...
          {size === "large" && (
            <div className="mt-2 text-xs text-gray-600">
              Financial trends chart visualization
            </div>
          )}
        </div>
      </div>
